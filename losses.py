...
class NerfWLoss(nn.Module):
    ...
    """
    def __init__(self, coef=1, lambda_u=0.01, color_optim=True, feat_optim=False):
        """
        ...
        self.color_optim = color_optim
        self.feat_optim = feat_optim

    def forward(self, inputs, targets, feat_targets=None):
        ret = {}
        if self.color_optim:
        ...
        if self.feat_optim:
            feat_targets = feat_targets.view(-1,feat_targets.shape[-1])
            ret['c_l_feat'] =  0.5 * ((inputs['feat_coarse']-feat_targets)**2).mean()
            if 'rgb_fine' in inputs:
                if 'beta' not in inputs: # no transient head, normal MSE loss
                        ret['f_l_feat'] = 0.5 * ((inputs['feat_fine']-feat_targets)**2).mean()
                else:
                    ret['f_l_feat'] = \
                        ((inputs['feat_fine']-feat_targets)**2/(2*inputs['beta'].unsqueeze(1)**2)).mean()
                    ret['b_l'] = 3 + torch.log(inputs['beta']).mean() # +3 to make it positive
                    ret['s_l'] = self.lambda_u * inputs['transient_sigmas'].mean()
